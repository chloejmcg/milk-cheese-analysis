...
There is one oddly long branch that I think we should investigate. 
...
tidytree::ancestor(unrooted_tree, 66) 
...
# Or if there are two nodes that we can see we can find its MRCA 
  ## let's do 66 and 162 for the purpose of getting some extra practice
...
There are a lot of sequences that came up, so this may be a legitimate sequence! But just to be sure let's BLAST it confirm our findings. 

# BLAST

Now, copy and paste the ASV sequences into NCBI’s Standard Nucleotide BLAST. 

Oddly enough, all the ASVs came back as probable sequences for the system. Primarily E. coli, Pseudomonas, Klebsiella, Acinetobacter, and Enterobacter. Our tree looks legitimate and we do not need to remove ASVs that were found. 

## 4b. Prune your tree

No ASVs need to be removed, so no nodes will be removed at this step--but here is where I could do that. 
...
Branch 66 looks to be a long but after inspecting the ASV and BLAST-ing the sequence, it does seem to be a legitimate potential contaminant in cheese. I am not going to remove an ASVs from my tree. 
